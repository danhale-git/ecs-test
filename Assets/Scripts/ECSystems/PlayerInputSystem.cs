...
public class PlayerInputSystem : ComponentSystem
{
    ...
    bool VoxelRay(Ray ray, out VoxelRayHit hit)
    {
        ...
        if(!managerSystem.mapMatrix.TryGetItemFromWorldPosition(previousVoxelOwnerPosition, out currentOwner))
        ...
        for(int i = 0; i < 1000; i++)
        {
            ...
            if(!previousVoxelOwnerPosition.Equals(nextVoxelOwnerPosition))
            {
                ...
                if(!managerSystem.mapMatrix.TryGetItemFromWorldPosition(nextVoxelOwnerPosition, out currentOwner))
                    continue;
